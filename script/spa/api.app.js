define(['jquery'], function ($) {
    app.performance = function () {
        ...
        app.domain = function () {
            var domain = {
                ...
                get: function (namespace, name) {
                    if (domain.scope[namespace]) {
                        return name === undefined ? domain.scope[namespace] : domain.scope[namespace][name];
                    }
                }
            };
        ...
        app.alert = function () {
            ...
            var SHOW_TYPE = {
                    SUCCESS: 'success',
                    success: 'aui-round_success alert-success',
                    _DEFAULT: 'info',
                    info: 'aui-Form-Info alert-info',
                    ERROR: 'error',
                    error: 'aui-round_error alert-pink',
                    WARNING: 'warning',
                    warning: 'aui-round_warming alert-warning',
                    PINK: 'pink',
                    pink: 'aui-round_error alert-pink',
                    MESSAGE: 'message'
                },
                alertCtnTemp = '<ul id="alertList" data-role="alertList" class="alert-list unstyled" style="z-index: 8;"></ul>',
                alertCttTemp = '<li data-alert-id="_id_"><i class="aui aui-guanbi alert-btn" title="关闭" data-role="close"></i><i class="aui aui-jiantou-xia alert-btn" data-role="more" title="更多"></i><i class="alert-icon aui _showType_"></i><div class="alert-content">_content_</div></li>',
                ...
                messageDialog = function () {
                    var queen = [],
                        clickHandler = function () {
                            ...
                            if (queen.length) {
                                msg = queen[0];
                                while (queen.length && !msg) {
                                    queen.shift();
                                    msg = queen[0];
                                }
                                if (msg) {
                                    $(this).find('[data-role=message]').html(msg.toString().replace(/\n/g, '<br/>'));
                                    result = false;
                                }
                            }
                            return result;
                    ...
                    if (type === SHOW_TYPE.MESSAGE) {
                        ...
                        if ($alert.children().length < alertQueueLength) {
                            if (!id || !$alert.children('[data-alert-id="' + id + '"]').length) {
                                $item = $alert
                                    .append(alertCttTemp.replace(/_id_/, id).replace(/_showType_/, type).replace(/_content_/, msg))
                                    .children(':last');
        ...
        app.dispatcher = (function () {
            ...
            Event.prototype = {
                            ...
                            if (namespace) {
                                ...
                                if(events){
                                    for (j = events.length; event = events[--j];) {
                                        if (event.namespace.match(new RegExp('\\b'+namespace+'\\b'))) {
                                            events.splice(j, 1);
                                            break;
                                        }
                                    }
                ...
                trigger: function (type, options, immediately) {
                    var context = this,
                        args = arguments;

                    if (this.timeout && args[args.length-1]!== true) {
        ...
        app.reset = function ($form) {
            $(':input', $form).not(':button, :submit, :reset,:radio,:checkbox,:disabled').val('').removeAttr('selected');
            $(':checked', $form).not(':disabled').removeAttr('checked');
        };
            ...
            full.triggerResizeHandler = function (uid, isGlobal) {
                if (uid) {
                    if (isGlobal) {
                        ...
                        resizeHandlerList[uid].callback && resizeHandlerList[uid].callback();
                    }
                }
            };
        ...
        app.popover = function () {
            ...
            popover = function () {
                ...
                function popover(options) {
                    ...
                    // var $popover =  $(options.$elem).closest('button').length !== 0 ? $(options.$elem).closest('button') : $(options.$elem).closest('span').length !== 0 ? $(options.$elem).closest('span') : $(options.$elem);
        ...
        app.select = function (options, undefined) {
            ...
            //监听绑定
            //多选按钮的更改事件
            __$btn.change(function () {
                var checked = this.checked,
                    checkedMethod = !checked ? 'removeAttr' : 'attr',
                    execMethod = checked ? _setNode : _removeNode, i, saveCheck = true;
        ...
        app.Controller = function () {
            ...
            View.prototype = {
                ...
                _default: {
                    ...
                    fixedButtonTemp: '<i class="fixed aui aui-guding3"></i>',
                ...
                open: function (options) {
                    ...
                    this.contextMenuOption.middle = fixed ? '' : {
                        name: '关闭此窗口',
                        filter: ':eq(_index_)'
                    };
                    ...
                    if (!options.type || options.type === TYPE.BLANK) {
                        ...
                        if ($tab.length) {
                            ...
                            if (this.stack.length > 1 && (handler = this.controller.getCacheHandler(this.stack[this.stack.length - 1])) && (handler.type === TYPE.SUB || handler.type === TYPE.POPOVER)) {
                                ...
                                this.$tabs.append(this.tabTemp.replace(/_domID_/, domID)
                                    .replace(/_id_/, id)
                                    .replace(/_href_/, href)
                                    .replace(/_title_/g, title)
                                    .replace(/_button_/, fixed ? this.fixedButtonTemp : this.closeButtonTemp)
                                );
                ...
                openWindow: function (options) {
                    ...
                    a.setAttribute("href", url + '?' + app.getNewQueryStringURL({
                            windowId: windowId
                        }));
                ...
                focusTab: (function () {
                    var _focusTab = function ($tab, $btn) {
                            var widths = 0,
                                ...
                                tabsContainerWidth = $tabCtn.innerWidth() - $tabBtn.outerWidth() * 2,
